...
`compute_advantages` メソッドを見ると、`returns` はすでに「リターン（割引累積報酬）」として正しく計算されています。

したがって、前回指摘した「Criticの学習対象をリターンに修正する」という方針は正しく、修正すべき箇所は `MAPPO.update` 内の Critic 更新部分のみです。
