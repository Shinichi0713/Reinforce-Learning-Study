...
![1786840301132](image/18_more_well_predator_v1/1786840301132.png)

エントロピにも低下＝エージェントの迷いが小さくなっていることが確認出来ます。

![1786840462250](image/18_more_well_predator_v1/1786840462250.png)

### MoEの結果

気になっていたエキスパートの分散について確認しました。
モジュールから収集した4体のエキスパートの利用率を学習開始段階と終了段階で比較しました。

グラフの見方ですが散布図の左側が学習開始時点で右側は学習終了時点の利用率の推移を示します。

![1786841611328](image/18_more_well_predator_v1/1786841611328.png)

__エキスパート利用率の変化・分析ポイント__

極端な偏り（Dead Expert / Dominant Expert）の解消が解消されました。

初期（Update 1001-1003）: decoder.layers.0.moe の Expert 2 は利用率が 0.01（約0.7%） とほぼ休眠状態（Dead Expert）であり、一方で Expert 3 に 0.52（約52%） が集中する極端な偏りがありました。

終盤（Update 1048-1050）: 同レイヤーの Expert 2 は 0.09〜0.12 まで復調し、特定エキスパートへの超過過重が解消されています。

__moe_aux_loss とロードバランスの収束__

学習進展に伴い補助損失（moe_aux_loss）が 1.063 → 1.011 へと減少しています。

これに連動して、全6レイヤー×4エキスパート（計24個）のデータポイントが、散布図上の理想点 (0.25, 0.25) 付近に集中的に収縮しており、ロードバランシング機能が正常に機能して全エキスパートが効率的に活用される状態へ移行したことが確認できます。
...
<img src="image/18_more_well_predator_v1/pursuit_mat_fixed (3).gif">

結局、敵を捕まえるには待ち構えることが良いということなのでしょうか。

## 総括

今回の取り組みと結果のについてまとめました。

__1. 仮説は部分的に間違っていた__

MoE collapse（特定expertへの偏り）が「敵が減った後も居座る」行動の原因だと仮説を立て、負荷分散損失とgateのLayerNormを導入した。結果として、**expertの使用分散は改善し、学習性能自体も向上**（捕獲数の増加・エントロピー低下）した。しかし、**行動パターンの切り替え（待機から探索へ）は起きなかった**。つまり、MoE collapseは実在したが、それが「居座り」の主因ではなかった。

__2. 待機戦略が最適解に近い可能性__

性能は向上したにも関わらずエージェントが「待ち構える」行動を選択し続けたことは、**その行動が与えられた観測情報と報酬設計の下で、実は合理的な戦略**であることを示唆している。敵が自ら近づいてくる環境（pursuit_v5）では、能動的に探索するより待機して包囲囲網を維持する方が累積報酬が高くなる可能性がある。

__3. 次に検証すべきは「観測情報の質」__

MoEの表現力を引き出しても、モデルに「切り替えるべき状況」を認識させる情報が入力されていなければ行動は変わらない。今回の結果は、**原因が観測情報の不足（例：担当ターゲントの明示的付与）や報酬の時間的スパース性にある**という仮説を強く支持する。MoEの構造改善ではなく、エージェントが「今誰を追うべきか」「敵がいないエリアを探索する価値」を認識できるよう、入力や報酬設計を改修する段階に移るべきである。
