...
赤色の味方同士が近づいていく傾向は見えますが、効果的に囲むような挙動にはなっていきません。

報酬は変えたので結構できばえは変わっているはずです。
結果は目に見える程度とは言いづらい状況でした。

<img src="image/9_reward_improvement/pursuit_mappo.gif">

## 総括

今回の取り組みを整理すると、以下のようになります。

### 課題と考えていたこと（チーム連携が弱い理由の仮説）

- 報酬が「敵の周囲にいること」や「隣接すること」を主に評価しているため、  
  各エージェントが**個別に最寄りの敵にまとわりつく**だけで報酬が得られてしまいます。
- その結果、
  - 共通の敵を**全員で狙う**インセンティブが弱い
  - 包囲網を**時間的に絞り込む**（距離を詰める）インセンティブが弱い
  - 4方向を**綺麗に分担する**インセンティブが弱い  
  といった問題が残っている可能性があります。

### チーム連携を強化する報酬系の改善案（コンパクト提案）

__1. 「共通の敵」を強調する報酬__

- 現状：各エージェントが「自分から見て最も近い敵」に反応
- 改善案：
  - **全エージェントが同じ敵をターゲットにしている**場合にボーナス報酬  
    （例：全員の `closest_prey_pos` が同一 or 近接している場合）
  - これにより、「バラバラに敵を追う」よりも「1体の敵を全員で追う」行動が強化されます。

__2. 包囲網の「時間的な絞り込み」を評価する報酬__

- 現状：敵の周囲にいること自体が報酬
- 改善案：
  - エピソードを通じて、**敵との平均距離が徐々に小さくなっている**場合に報酬  
    （例：ステップごとに「敵との距離の減少量」を追加報酬）
  - これにより、「囲み続けるだけ」ではなく、**包囲網を絞り込む行動**が強化されます。

__3. フォーメーションの「分担」をより厳密に評価__

- 現状：`ally_layer == 1` で重複を抑制
- 改善案：
  - 敵を中心とした**4方向（上下左右）に味方が1体ずついる**場合に、  
    追加の強い報酬（例：`cross_position_reward_scale` を大きくする）
  - これにより、「4方向を綺麗に分担する」フォーメーションが明示的に評価されます。

__4. 「囲み続けるだけ」の報酬ハックを防ぐ__

- 現状：囲んでいるだけで毎ステップ報酬が入る
- 改善案：
  - 一定ステップ以上同じ敵を囲み続けていると、**包囲報酬が減衰**する  
    （例：`shaping_reward *= decay_factor`）
  - これにより、「捕獲せずに囲み続ける」インセンティブを弱め、  
    実際の捕獲行動を促します。

### まとめ

- これまでの報酬設計で、**敵の周囲に集まる・隣接する**行動はある程度学習できています。
- しかし、**4人が協調して包囲網を絞り込む「組織立った囲み」** まではまだ到達していないため、  
  - 共通の敵を強調する報酬  
  - 包囲網の時間的な絞り込みを評価する報酬  
  - フォーメーション分担を厳密に評価する報酬  
  - 囲み続けるだけの報酬ハックを防ぐ減衰  
  といった追加の報酬設計が有効かもしれません。
  ただし、過去の強化学習で画像からの学習を行わせようとした場合、かなり学習が安定しなかったという実績がありました。
  別アプローチも検討すべきかと考えた今回の結果でした。
