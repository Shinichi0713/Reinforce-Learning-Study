    ...
    def _obs_to_tensor(self, obs: Dict[int, Tuple], is_state: bool = False):
        if is_state:
            ...
            for i in range(self.n_agents):
                # obs[i][0] が (x, y) タプルであることを前提とする
                # もしタプルでなく単一の値になっているなら、タプルに戻すか、処理を変える必要がある

                # --- 修正/確認: 位置情報をタプルとして展開 ---
                pos_tuple = obs[i][0] # ここでタプル (x, y) が入っているはず

                # エラーが出た行を以下のように修正 (x, y) を明示的にアンパック
                state_vec.extend([pos_tuple[0] / (GRID_SIZE - 1),
                                  pos_tuple[1] / (GRID_SIZE - 1)])

                # 元のコード
                # state_vec.extend([x / (GRID_SIZE - 1) for x in obs[i][0]]) # ここでエラー
