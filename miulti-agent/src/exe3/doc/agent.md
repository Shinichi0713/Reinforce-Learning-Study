この「災害現場の探索」タスクは、状態空間（マップの組み合わせ）が **$2^{225}$** 通りと非常に膨大であるため、先ほどの倉庫タスクで用いた単純なQテーブル（表形式）では学習が困難です。

そこで、**Deep Q-Network (DQN)** を用いたアプローチを採用します。

### 🧠 アプローチの概要：畳み込みニューラルネットワーク (CNN) + 共有DQN

1. **CNNの利用:** マップはグリッド画像のようなものなので、画像の処理が得意な **CNN (Convolutional Neural Network)** を使って、「どこが未探索か」という空間的な特徴を抽出します。
2. **パラメータ共有 (Shared Parameter):** 3台のドローンは全く同じ機能を持っています。それぞれに別々の脳を作るのではなく、**1つの強力な脳（ニューラルネットワーク）を全員で共有**します。これにより、学習データが3倍の効率で集まり、学習が高速化します。
3. **入力データ:** ニューラルネットワークへの入力は、画像のように **2つのチャンネル** を重ねて渡します。
   * Ch1:  **探索済みマップ** （全体共有情報）
   * Ch2:  **自分の位置マップ** （そのエージェントがどこにいるか）

---

### 💻 実装コード (PyTorch使用)

このコードを実行するには `torch` (PyTorch) が必要です。Google Colab等であれば標準でインストールされています。
...
* 3機のドローンで1つのニューラルネットワークを共有します。
* `preprocess_state` メソッドが重要です。ここで、環境からの辞書データ（位置情報とマップ）を、CNNが理解できる「2チャンネルの画像データ」に変換しています。
  * 1枚目：どこが未探索か？（全体マップ）
  * 2枚目：自分はどこにいるか？（One-hotエンコーディング）

1. **`DuelingDQN` クラス** :

* 入力された画像を畳み込み層（`Conv2d`）に通し、空間的な特徴（例：「右上に大きな未探索エリアがある」など）を抽出します。

1. **学習ループ** :

* 各ステップで、3機分の経験 `(状態, 行動, 次の状態, 報酬)` をメモリに保存し、ランダムにサンプリングして学習を行います。
