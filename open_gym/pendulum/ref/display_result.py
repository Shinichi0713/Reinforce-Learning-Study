...
def display_result_loss(loss_actor, loss_critic):
    # 結果をプロット
    plt.plot(np.arange(len(loss_actor)), loss_actor, label='Actor Loss')
    plt.plot(np.arange(len(loss_critic)), loss_critic, label='Critic Loss')
    plt.xlabel('Episode')
    plt.ylabel('Loss')
    plt.title('Training Result(loss shift of SAC)')
    plt.legend()
...
def display_result_reward(result):
    ...
    plt.xlabel('Episode')
    plt.ylabel('Reward')
    plt.title('Training Result(reward shift of SAC)')
    dir_current = os.path.dirname(os.path.abspath(__file__))
    path_reward_sac = os.path.join(dir_current, "reward_history_sac.png")
