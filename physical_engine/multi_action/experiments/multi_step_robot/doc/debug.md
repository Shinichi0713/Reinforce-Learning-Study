...
pdbでデバッグ対話を終了したい場合は、以下のいずれかを入力します。

- `q`（quitの短縮形）
- `quit`

どちらも同じ意味で、**デバッガを終了してプログラムの実行を中断**します。
...
`pdb.set_trace()` で止まった状態で、pdbプロンプトに

```
(Pdb) q
```

と入力すると、デバッガが終了し、プログラムもそこで止まります。

---

## 注意点

- `q` / `quit` は **デバッガを終了し、プログラムもそこで中断** します。
- 単にブレークポイントを抜けて実行を続けたい場合は、`c`（continue）を使います。
  - `c`：次のブレークポイントまで実行を続ける
- デバッガを終了せずに1行ずつ進めたい場合は、`n`（next）や`s`（step）を使います。

---

## よく使うpdbコマンド一覧（参考）

- `n`（next）：次の行へ（関数の中には入らない）
- `s`（step）：関数の中へ入る
- `c`（continue）：次のブレークポイントまで進む
- `p <変数名>`：変数の値を表示
- `l`（list）：現在の周辺コードを表示
- `q` / `quit`：デバッガを終了してプログラムを中断

ということで、**対話を終了したいときは `q` または `quit`** と入力してください。